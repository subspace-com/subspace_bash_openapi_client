...
# ! Installation:
# !
# ! Copy the _subspace-client.sh file to any directory under FPATH
  ...
  args)
    case $line[1] in
      acceleratorServiceCreate)
        ...
        _op_arguments=(
                              "Idempotency-Key\::[HEADER] Value is the returned etag of a get request.  If a retry sends an Idempotency-Key that has been seen before, the existing accelerator is returned with the status code of 200"
)
